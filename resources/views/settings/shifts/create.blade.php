                ...
                <div class="bg-white shadow-md rounded my-6 p-5">
                    ...
                    <form method="POST" action="{{ route('admin.shifts.store') }}">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}">{{ $user->first_name }} - {{ $user->roles->first()->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="text-center mt-16">
                            <button type="submit"
                                class="bg-[#221551] text-white font-bold px-5 py-1 rounded focus:outline-none shadow hover:bg-[#6E38D5] transition-colors ">Save</button>
                        </div>
                    </form>
                </div>
            </div>
